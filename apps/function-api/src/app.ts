...
import { FunctionStore } from "./runtime/executor.js";
import { createFunctionRouter } from "./routes/v1.js";

export function createApp(options?: {
  logger?: Logger;
  store?: FunctionStore;
}): { app: Express; logger: Logger; store: FunctionStore } {
  const startedAt = Date.now();
  const logger = options?.logger ?? createLogger({ service: "function-api" });
  const store = options?.store ?? new FunctionStore();
  const app = express();
  ...
  app.get("/ready", (_req, res) => {
    res.json({ status: "ready", service: "function-api" });
  });

  app.use("/api/v1/functions", createFunctionRouter(store));
