...
export async function ensureSchema(client: { query: pg.Pool["query"] }, schema: string): Promise<void> {
  await client.query(`CREATE SCHEMA IF NOT EXISTS ${quoteIdent(schema)}`);
}

export function assertSafePhysicalRef(schema: string | null, table: string): {
  schema: string | null;
  table: string;
  qualified: string;
} {
